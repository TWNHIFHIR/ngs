...
* component[representative-coding-hgvs] ^definition = "人類基因組變異協會（HGVS）命名法用於描述1一 DNA 標誌（marker）。此命名法需要擴充以描述非變異型（即wild type），可參考範例以了解wild types的表述方式。"
...
* component[representative-coding-hgvs].valueCodeableConcept.code from GeneTestList
* component[representative-coding-hgvs].valueCodeableConcept.code.coding 1..1
